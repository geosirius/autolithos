...
if uploaded_file and len(pozos_data) > 0:
    ...
    if len(orden_pozos) > 0:
        fig = graficar_clustering_y_curvas(orden_pozos, clustering_results, curvas_normalizadas, profundidad_min, profundidad_max, clusters_filtrados, curvas_seleccionadas)
        st.pyplot(fig)
    else:
