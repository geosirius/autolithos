...
st.title("Clustering Geológico de Registros de Pozos")
st.markdown("""
Bienvenido a la aplicación **Clustering Geológico de Pozos**.

Esta herramienta te permite realizar clustering no supervisado sobre datos de registros de pozos en formato LAS. A través de este análisis podrás visualizar patrones en las curvas y analizar la continuidad de clusters entre pozos cercanos.

**Características:**
- Carga de múltiples archivos LAS comprimidos en un archivo ZIP.
- Selección de algoritmos de clustering (KMeans, GMM, DBSCAN).
- Opción de normalización de curvas para una mejor visualización.
- Visualización de la continuidad de los clusters entre pozos.
""")

# Función para leer archivos LAS desde ZIP
def leer_archivos_las(uploaded_file):
    pozos_data = {}
    try:
...
if uploaded_file and len(pozos_data) > 0:
    ...
    n_clusters = st.sidebar.slider("Número de clusters para KMeans y GMM", 2, 10, 5)
    ...
    for pozo in pozo_nombres:
        ...
        # Aplicar el algoritmo de clustering seleccionado
    ...
    # Selección de clusters para visualizar
